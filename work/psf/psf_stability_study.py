...
for filename in args.psf :
    ...
    expid=int(vals[-1])
    cam=vals[-2]
    preproc=os.path.join(os.path.dirname(filename).replace("exposures","preproc"),"preproc-{}-{:08d}.fits".format(cam,expid))
    header = fitsio.read_header(preproc)
    expids.append(expid)
...
for fiber in fibers :
    for wave in waves :
        ...
        for psf,header in zip(psfs,headers) :
            ...
            cumul_expid.append(header["EXPID"])
            cumul_mjd.append(header["MJD-OBS"])
