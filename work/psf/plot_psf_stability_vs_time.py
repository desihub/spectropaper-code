...
mjd0=np.min(table["mjd"])
hours=(table["mjd"]-mjd0)*24.
...
#for wave in [ waves[waves.size//2] ] :
    ...
    for fiber in fibers :
        ...
        a_x.plot(hours[selection],table["x"][selection]-mx,lstyle,color=color,alpha=alpha)
        a_y.plot(hours[selection],table["y"][selection]-my,lstyle,color=color,alpha=alpha)
        mxsig=np.mean(table["xsig"][selection])
        mysig=np.mean(table["ysig"][selection])
        a_xsig.plot(hours[selection],table["xsig"][selection]/mxsig-1.,lstyle,color=color,alpha=alpha)
        a_ysig.plot(hours[selection],table["ysig"][selection]/mysig-1.,lstyle,color=color,alpha=alpha)
        memm=np.mean(table["emission_line_coeff"][selection])
        a_emm.plot(hours[selection],table["emission_line_coeff"][selection]-memm,lstyle,color=color,alpha=alpha)
...
a_xsig.grid()
a_ysig.grid()
a_xsig.set_xlabel("time delay (hours)")
a_ysig.set_xlabel("time delay (hours)")


a_emm.grid()
a_emm.set_xlabel("time delay (hours)")
